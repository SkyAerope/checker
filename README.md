...
![前端长这个样子](images/image.png)

### 怎样的数据？
怎样的数据都可以。数据可以是一个或者多个。
...
可选：后端还可以暴露一个`/health`接口用于检测连接。此接口需接收GET请求，并返回`200`状态码。
...
pnpm build
```
构建产物在`frontend/dist`目录下，是静态页面。

## 一些有的没的

### 用途
既然这只是一个模板，理论上你可以用它做各种事情。比如：
- 向数据库添加一些数据
- 查一下这些数据在不在数据库中
- 对接搜索引擎，并测试一些关键词能不能搜到你的网站
- 像1keys那样给sheerid链接做认证
